...
The estimated treatment effect in this model with all controls is 0.585611, which is slightly lower than the estimated treatment effect of the model that only includes `numhh_list` (difference of 0.007889). We have a similar history with the standard error: when comparing both both models we get a difference of only 0.003. Finally, the $R^2$ of this model is 0.03257, which is higher significantly higher than the $R^2$ of 0.006276 in the model with only `numhh_list`, indicating that including all baseline characteristics as controls improves the fit of the model and explains more of the variation in the `count_visit_dr` variable.

### Q4. Code
```{r q4_model5, include=TRUE, echo=TRUE, collapse=TRUE}
model_5 <- lm(count_visit_dr ~ treated + numhh_list + female + age + race_white + hs_degree + college_degree + health_baseline, data=df, na.action = na.omit)
summary(model_5)
```
...
For the alternative hypothesis: $$H_A: \beta_{treated} \neq \beta_{hs\_degree}$$.

Using a F-test (code and results below), we fail to reject the $H_0$. The F-statistic is 0.90, and the critical value at the 5% significance level is 3.84. Since the F-statistic is less than the critical value, we do not have enough evidence to reject the null hypothesis.

### Q7. Code
```{r q7ftest, include=TRUE, echo=TRUE, collapse=TRUE}

#unrestricted model (the same from q4)
unr_model <- lm(count_visit_dr ~ treated + numhh_list + female + age + race_white + hs_degree + college_degree + health_baseline, data = df, na.action = na.omit)
unr_SSR <- sum(unr_model$residuals^2)

#restricted model
df$combined <- df$treated + df$hs_degree
res_model <- lm(count_visit_dr ~ combined + numhh_list + female + age + race_white + college_degree + health_baseline, data = df, na.action = na.omit)
res_SSR <- sum(res_model$residuals^2)

q <- 1       
n <- nobs(unr_model)
k <- length(coef(unr_model)) - 1

F2 <- ((res_SSR - unr_SSR) / q) / (unr_SSR / (n - k - 1))
F2
```

```{r q7cv, include=TRUE, echo=TRUE, collapse=TRUE}
#critical value for test
qf(.95, df1 = q, df2 = n - k - 1)
```
```{r q7pvalue, include=TRUE, echo=TRUE, collapse=TRUE}
#computing p-value for tes
pf(F2, q, n - k - 1, lower.tail = FALSE)
```
